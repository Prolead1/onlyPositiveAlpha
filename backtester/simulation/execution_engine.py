    ...
    def simulate_hold_to_resolution_backtest_incremental(  # noqa: C901, PLR0912, PLR0913, PLR0915
        ...
        *,
        ...
        total_markets = int(
            pd.DataFrame(
                {
                    "market_id": signal_frame["market_id"].astype(str),
                    "feature_date": signal_feature_dates,
                }
            )
        ...
        # Build composite lookup: (market_id, feature_date) -> resolution row.
        # Keep a single-row fallback per market for legacy resolution files that
        # predate feature_date.
        resolution_lookup_dict: dict[tuple[str, str], dict[str, object]] = {}
        market_resolution_candidates: dict[str, list[dict[str, object]]] = {}
        for _, row in resolution_frame.iterrows():
            market_value = row.get("market_id")
            if pd.isna(market_value):
                market_value = row.name
            market_key = str(market_value).strip()
            if not market_key:
                continue

            resolved_val = pd.to_datetime(row.get("resolved_at"), utc=True, errors="coerce")
            if pd.isna(resolved_val):
                continue

            feature_date_raw = row.get("feature_date")
            if pd.isna(feature_date_raw):
                feature_date_key = resolved_val.strftime("%Y-%m-%d")
            else:
                parsed_feature_date = pd.to_datetime(
                    feature_date_raw,
                    utc=True,
                    errors="coerce",
                )
                if pd.isna(parsed_feature_date):
                    feature_date_key = str(feature_date_raw).strip()
                else:
                    feature_date_key = parsed_feature_date.strftime("%Y-%m-%d")

            resolution_row_payload = row.to_dict()
            resolution_row_payload["market_id"] = market_key
            resolution_row_payload["resolved_at"] = resolved_val
            resolution_row_payload["feature_date"] = feature_date_key

            composite_key = (market_key, feature_date_key)
            existing_payload = resolution_lookup_dict.get(composite_key)
            if existing_payload is None:
                resolution_lookup_dict[composite_key] = resolution_row_payload
            else:
                existing_resolved = pd.to_datetime(
                    existing_payload.get("resolved_at"),
                    utc=True,
                    errors="coerce",
                )
                if pd.isna(existing_resolved) or resolved_val < existing_resolved:
                    resolution_lookup_dict[composite_key] = resolution_row_payload

            market_resolution_candidates.setdefault(market_key, []).append(
                resolution_row_payload
            )

        market_fallback_lookup: dict[str, dict[str, object]] = {}
        for market_key, candidates in market_resolution_candidates.items():
            unique_resolved_values = {
                pd.to_datetime(candidate.get("resolved_at"), utc=True, errors="coerce")
                for candidate in candidates
            }
            unique_resolved_values = {
                value for value in unique_resolved_values if not pd.isna(value)
            }
            if len(unique_resolved_values) != 1:
                continue
            market_fallback_lookup[market_key] = min(
                candidates,
        ...
        markets_with_resolution_set = set(market_resolution_candidates)

        trade_rows: list[dict[str, object]] = []
        for market_id_str, market_group_all in self._iter_market_groups_by_id(signal_frame):
            if market_group_all.empty:
                continue

            group_feature_dates = pd.Series(
                pd.to_datetime(market_group_all.index, utc=True, errors="coerce"),
                index=market_group_all.index,
            ).dt.strftime("%Y-%m-%d")
            market_group_with_dates = market_group_all.copy()
            market_group_with_dates["_feature_date"] = group_feature_dates

            for feature_date_str, market_group in market_group_with_dates.groupby(
                "_feature_date",
                sort=True,
                dropna=True,
            ):
                ...
                if market_id_str not in markets_with_resolution_set:
                    _update_progress_and_metrics()
                    continue

                resolution_row = resolution_lookup_dict.get(
                    (market_id_str, str(feature_date_str))
                )
                if resolution_row is None:
                    resolution_row = market_fallback_lookup.get(market_id_str)
                if resolution_row is None:
                    _update_progress_and_metrics()
                    continue
                ...
                resolved_at = pd.to_datetime(
                    resolution_row.get("resolved_at"),
                    utc=True,
                    errors="coerce",
                )
                ...
                market_group = market_group.drop(columns=["_feature_date"], errors="ignore")
                ...
                if lookahead_seconds > 0:
                    ...
                    entry_candidates = eligible.loc[
                        (eligible.index >= anchor_entry_ts)
                        & (eligible.index <= lookahead_end_ts)
                    ].copy()
                else:
